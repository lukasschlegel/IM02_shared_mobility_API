...
function datenInArray(data) {
    ...
    data.forEach( scooter => {
        let scooterObj = {
          ...
          properties: {
            title: 'Mapbox',
            description: `<strong>Scooter</strong>
                          <br>
...
async function initMap(lat = 8.536919584914061, long = 47.38059039878863, redrawInput = true) {
  ...
  drawMap(mapBoxArray, lat, long, redrawInput)

  console.log(redrawInput);
}
initMap();

let geocoder; 

function drawMap(pointArray, lat = 8.538, long = 47.38, redrawInput = true) {
  ...
  geocoder.on('result', (selected) => {
    initMap(selected.result.center[0], selected.result.center[1], false);
  }) 
